...
def validate_password_login(db: Session, user_id, password):
    result = db.execute(text('SELECT password FROM users WHERE user_id = :user_id'), {
...
def add_user(body: AuthRegisterSchema, db: Session = Depends(get_db)):
    ...
    result = db.execute(text('SELECT 1 FROM users WHERE user_id = :user_id'), {
                        'user_id': user_id}).scalar()
    if result:
        raise HTTPException(
            status_code=409, detail=f'User with id {user_id} is already exist')
...
def log_user(body: AuthLoginSchema, db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(409, 'Invalid credencial')
    if not validate_password_login(db, user_id, password):
        raise HTTPException(status_code=401, detail='Invalid credencial')
    token = user.generate_jwt()
    return {
        'success': True,
        'token': token
