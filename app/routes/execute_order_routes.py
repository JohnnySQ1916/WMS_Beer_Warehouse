...
@router.get("/order_choice", response_model=ApiResponse)
...
def check_products_on_order(
    ...
    products = db.execute(products_query, {"order_id": order_id}).fetchall()
    if not products:
        raise HTTPException(status_code=404, detail="Order not found")
    return ApiResponse(
        data={
            "Products": [
                {"Product name": row.product_name, "Amount": row.amount} for row in products
            ]
        }
    )


@router.get("/start_order/{order_id}", response_model=ApiResponse)
def start_order(
    order_id: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
...
@router.post("/confirm_location/{order_id}", response_model=ApiResponse)
def confirm_location(
    ...
    return ApiResponse(message=f"Enter ean: {first.ean}")
...
def confirm_amount(
    ...
    collected = product_service.fetch_scalar(
        "collected_amount", {"ean": first.ean, "order_id": order_id}, "orders_details"
    )
    ...
    if amount_on_location - (amount + collected) < 0:
        return ApiResponse(message=f"It is only {amount_on_location} amount on location. Try again")
    if amount + collected > first.amount:
        return ApiResponse(message="Too big number to take. Try again")
    execute.take_product_out_of_base(order_id, first, amount, collected, user_id)
    if amount + collected == first.amount:
        return ApiResponse(message="Product fully completed. Get next product")
    if amount + collected < first.amount:
        return ApiResponse(message=f"You took already {amount + collected} units.")


@router.get("/get_next_product/{order_id}", response_model=ApiResponse)
def get_next_product(
    ...
    if not order:
        ...
        return ApiResponse(
            ...
            data={
                "Picked products": [
                    {"Product name": row.product_name, "Amount": row.picked_amount} for row in done
                ]
            },
        )
    first = order[0]
    amount_available = product_service.fetch_scalar(
        "amount",
        {"ean": first.ean, "location": first.location, "date": first.date},
        "products",
    )
...
@router.post("/reverse_product/{order_id}", response_model=ApiResponse)
def reverse_product(
    order_id: str,
    body: ChooseProductSchema,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    execute = ExecuteOrder(db)
    ...
    if not product:
        raise HTTPException(
            status_code=404,
            detail=f"Product with id {product_id} not found for order {order_id}",
        )
    try:
        execute.reverse_picked_product_out_of_base(order_id, product)
        return ApiResponse(message="Picked product has been reverse")
