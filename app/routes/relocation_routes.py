...
def get_products_by_ean(
    ean: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
...
def enter_amount(
    ...
    amount_on_location = product_service.fetch_scalar(
        "amount", {"ean": ean, "location": location, "date": date}, "products"
    )
    if amount > amount_on_location:
        return ApiResponse(
            message=f"Too high number to relocate. On location it is only {amount_on_location}."
        )
    else:
...
def enter_target_location(
    ...
    try:
        ...
        relocation_service.relocate_in_products(
            ean, location, date, amount, target_location
        )
        return ApiResponse(message="Relocate confirmed")
...
def get_products_by_location(
    location: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    location_service = LocationService(db)
    relocation_service = RelocationService(db)
...
def enter_ean(
    ...
    dooble = db.execute(dooble_query, {"ean": ean, "location": location}).fetchall()
    if len(dooble) > 1:
        return ApiResponse(
            message="Choose product to relocate",
            data=[
        ...
        raise HTTPException(
            status_code=400,
            detail=f"No product found with EAN {ean} on location {location}.",
        )
