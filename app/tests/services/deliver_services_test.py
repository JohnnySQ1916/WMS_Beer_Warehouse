...
def prepare_test_data(db_session):
    ...
    for (
        ...
        db_session.execute(
            text("""INSERT INTO suppliers (company_name, contact_name, contact_title, address, city, region, 
...
def test_supplier_exist(db_session, prepare_test_data):
    ...
    supplier1 = "New Orleans Cajun Delights"
    supplier2 = "Firma Krzak"
    assert delivery_service.supplier_exist(supplier1)
    assert not delivery_service.supplier_exist(supplier2)
...
def test_create_supplier_deliver(db_session, prepare_test_data):
    ...
    result = db_session.execute(
        text("SELECT * FROM delivery_order WHERE deliver_id = :id"), {"id": id}
    ).fetchone()
...
def test_create_deliver_details(
    db_session,
    prepare_test_data,
    code,
    product_name,
    ...
    result = db_session.execute(
        text("SELECT * FROM  deliver_details WHERE deliver_id = :id AND ean = :ean"),
        {"id": id, "ean": ean},
    ).fetchone()
...
def test_update_products(
    ...
    delivery_service = DeliveryService(db_session)
    user_id = "ks"
    new_item = ProductDetails(
        code=code, product_name=product_name, ean=ean, unit_weight=unit_weight
    )
    ...
    db_session.execute(
        text(
            "UPDATE deliver_details SET amount = :new_amount, date = :date WHERE ean = :ean AND deliver_id = :deliver_id"
        ),
    ...
    result = db_session.execute(
        text("SELECT * FROM products WHERE ean = :ean AND location = :target_location"),
        {"ean": ean, "target_location": target_location},
    ).fetchone()
    assert result.product_name == product_name
