...
def test_new_record_relocation(
    db_session,
    code,
    product_name,
    ean,
    ...
    result = db_session.execute(
        text("SELECT * FROM relocation WHERE ean = :ean"), {"ean": ean}
    ).fetchone()
...
@parametrize_decorator
def test_confirm_target_location(
    db_session,
    code,
    product_name,
    ean,
...
def test_relocate_in_products(
    ...
    relocated_product = db_session.execute(
        text(
            "SELECT * FROM products WHERE ean = :ean AND date = :date AND location = :target_location"
        ),
        {"ean": ean, "date": date, "target_location": target_location},
    ).fetchone()
...
@parametrize_decorator
def test_new_record_relocation_by_location(
    db_session,
    code,
    product_name,
    ean,
    ...
    result = db_session.execute(
        text("SELECT * FROM relocation WHERE initial_location = :location AND id = :id"),
        {"location": location, "id": new_id},
    ).fetchone()
    assert result is not None
...
def test_confirm_ean(
    ...
    result = db_session.execute(
        text("SELECT * FROM relocation WHERE id = :id"), {"id": new_id}
    ).fetchone()
    assert result is not None
    assert result.status == "date_confirmed"
    assert result.product_name == product_name
