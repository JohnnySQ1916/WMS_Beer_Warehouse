...
def test_create_order(db_session, prepare_test_data):
    ...
    result_orders = db_session.execute(
...
def test_insert_single_product_into_order_details(
    db_session,
    prepare_test_data,
    code,
    product_name,
    ...
    result = db_session.execute(
        text("SELECT * FROM orders_details WHERE ean = :ean AND order_id = :order_id"),
...
def test_add_customer_to_order(db_session, prepare_test_data):
    ...
    result = db_session.execute(
        text("SELECT customer_id FROM orders WHERE order_id = :order_id"),
        {"order_id": order_id},
    ).scalar()
...
@parametrize_decorator
def test_make_reservation_to_order(
    db_session,
    prepare_test_data,
    code,
    product_name,
    ...
    result_reservation = db_session.execute(
