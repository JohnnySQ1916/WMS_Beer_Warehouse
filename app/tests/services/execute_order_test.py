...
def test_reservation_by_create_order(
    db_session,
    code,
    product_name,
    ean,
    ...
    reservation = db_session.execute(
        text("SELECT * FROM reservation WHERE ean = :ean"), {"ean": ean}
    ).fetchone()
    order_amount = db_session.execute(
        text("SELECT amount FROM orders_details WHERE ean = :ean AND order_id = :order_id"),
        {"ean": ean, "order_id": order_id},
    ).scalar()
...
@parametrize_decorator
def test_reservation_of_location(
    db_session,
    code,
    product_name,
    ean,
    ...
    result = db_session.execute(
        text("""SELECT p.amount, p.reserved_amount, p.available_amount FROM products p JOIN orders_details od ON p.ean = od.ean 
    ...
    product = db_session.execute(
        text("SELECT * FROM products WHERE ean = :ean"), {"ean": ean}
    ).fetchone()
    amount_order = db_session.execute(
        text("SELECT amount FROM orders_details WHERE order_id = :order_id AND ean = :ean"),
        {"order_id": order_id, "ean": ean},
    ).scalar()
...
def test_insert_into_picks(
    ...
    picks = db_session.execute(
        text("SELECT * FROM picks WHERE order_id = :order_id AND ean = :ean"),
        {"order_id": order_id, "ean": ean},
    ).fetchone()
    amount_order = db_session.execute(
        text("SELECT amount FROM orders_details WHERE order_id = :order_id AND ean = :ean"),
...
@parametrize_decorator
def test_take_product_out_of_base(
    db_session,
    code,
    product_name,
    ean,
    ...
    user_id = "KS"
    execute_service.reservation_of_location(order_id)
    products_before_taken = db_session.execute(
        text("SELECT amount FROM products  WHERE ean = :ean"), {"ean": ean}
    ).scalar()
    product = db_session.execute(
        text("""SELECT od.product_name, od.code, od.amount, od.ean, od.status, p.id, p.location, p.date, p.available_amount
    ...
    products_after_taken = db_session.execute(
        text("SELECT amount FROM products  WHERE ean = :ean"), {"ean": ean}
    ).scalar()
    order_picking = db_session.execute(
        text(
            "SELECT * FROM order_picking_details WHERE order_id = :order_id AND expected_ean = :ean"
        ),
        {"order_id": order_id, "ean": ean},
    ).fetchone()
    order_details = db_session.execute(
        text("SELECT * FROM orders_details WHERE order_id = :order_id AND ean = :ean"),
        {"order_id": order_id, "ean": ean},
    ).fetchone()
    reservation = db_session.execute(
        text("SELECT * FROM reservation WHERE ean = :ean"), {"ean": ean}
...
@parametrize_decorator
def test_delete_row_from_products(
    db_session,
    code,
    product_name,
    ean,
    ...
    product1 = db_session.execute(
        text("SELECT * FROM products WHERE ean = :ean AND location = :location AND date= :date"),
        {"ean": ean, "location": location, "date": date},
    ).fetchone()
    execute_service.delete_row_from_products(product1)
    result = (
        db_session.execute(
            text("SELECT * FROM products WHERE ean = :ean AND location = :location"),
            {"ean": ean, "location": location},
        )
        .mappings()
        .all()
    )
...
@parametrize_decorator
def test_update_when_order_done(
    db_session,
    code,
    product_name,
    ean,
...
def test_get_done_products(db_session):
    ...
    for row in done:
        result = db_session.execute(
            ...
            {"order_id": order_id, "ean": row.expected_ean},
        ).fetchone()
...
@parametrize_decorator
def test_revers_orders_details(
    db_session,
    code,
    product_name,
    ean,
    ...
    product2 = product_service.fetch_one(
        {"order_id": order_id, "product_id": product.id}, "order_picking_details"
    )
    execute_service.take_product_out_of_base(order_id, product, amount_order, 0, user_id)
    product2 = product_service.fetch_one(
        {"order_id": order_id, "product_id": product.id}, "order_picking_details"
    )
    collected = db_session.execute(
        text(
...
def test_revers_reservation(
    ...
    execute_service.take_product_out_of_base(order_id, product, amount_order, 0, user_id)
    product_id = product_service.fetch_scalar(
        "id", {"ean": ean, "location": location, "date": date}, "products"
    )
    product2 = product_service.fetch_one(
        {"order_id": order_id, "product_id": product_id}, "order_picking_details"
    )
    order_details = db_session.execute(
        text("SELECT * FROM orders_details WHERE order_id = :order_id AND ean = :ean"),
        {"order_id": order_id, "ean": ean},
...
@parametrize_decorator
def test_reverse_picked_products(
    db_session,
    code,
    product_name,
    ean,
    ...
    order_picking = db_session.execute(
        text(
            "SELECT * FROM order_picking_details WHERE order_id = :order_id AND product_id = :product_id"
        ),
        {"order_id": order_id, "product_id": product_id},
    ).fetchone()
    product_amount = db_session.execute(
        text("SELECT amount FROM products WHERE ean = :ean"), {"ean": ean}
    ).scalar()
