...
def test_get_product_on_location(db_session, client, token, code, product_name, ean, amount, jednostka, unit_weight, location, date, reserved_amount, available_amount):
    prepare_db(db_session)
    response = client.get(f'/api/v1/location/{location}', headers={"Authorization": f"Bearer {token}"})
    expected_result = [{
        'code': code,
