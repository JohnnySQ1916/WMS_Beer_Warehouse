...
def test_get_product_by_ean(
    ...
    response = client.get(
        f"/api/v1/relocation/start/{ean}", headers={"Authorization": f"Bearer {token}"}
    )
    product = [
        {
            "code": code,
    ...
    relocation_id = db_session.execute(
        text("SELECT id FROM relocation WHERE ean = :ean ORDER BY id DESC LIMIT 1"),
        {"ean": ean},
    ).scalar()
...
def test_enter_location(
    ...
    response = client.post(
        f"/api/v1/relocation/enter_location/{relocation_id}",
        headers={"Authorization": f"Bearer {token}"},
        json={"location": location},
    )
    response_data = response.json()
    dooble = db_session.execute(
        text(
    ...
    if len(dooble) == 1:
        ...
        assert (
            response_data["message"] == f"No product found with EAN {ean} on location {location}."
        )
...
def test_enter_amount(
    ...
    new_amount = 40
    response = client.post(
        f"/api/v1/relocation/confirm_amount/{relocation_id}",
        headers={"Authorization": f"Bearer {token}"},
    ...
    if new_amount > amount:
        assert (
            ...
            == f"Too high number to relocate. On location it is only {amount}."
        )
...
@parametrize_decorator
def test_enter_target_location(
    db_session,
    client,
    token,
    code,
    ...
    db_session.execute(
        text("SELECT status FROM relocation WHERE id = :id"), {"id": relocation_id}
    ).scalar()
    target_location = "RB-01-01"
    response = client.post(
        f"/api/v1/relocation/confirm_target_location/{relocation_id}",
        headers={"Authorization": f"Bearer {token}"},
...
@parametrize_decorator
def test_get_product_by_location(
    db_session,
    client,
    token,
    code,
    ...
    relocate_service = RelocationService(db_session)
    response = client.get(
        f"/api/v1/relocation/relocate_by_location/{location}",
        headers={"Authorization": f"Bearer {token}"},
    )
    response_data = response.json()
    user_id = "ks"
...
#     response = client.post(
#         f"/api/v1/relocation/enter_ean/{relocation_id}",
#         headers={"Authorization": f"Bearer {token}"},
#         json={"ean": ean},
#     )
#     response_data = response.json()
