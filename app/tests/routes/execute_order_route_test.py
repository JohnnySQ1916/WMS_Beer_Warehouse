...
def test_execute_order_choice_positive(db_session, client, token):
    ...
    response = client.get("/api/v1/execute_order/order_choice", headers={"Authorization": f"Bearer {token}"})
    response_data = response.json()
    assert response.status_code == 200
    assert response_data['message'] == """Check order: If you want to check products on order, enter Check order and put order number to the link,
        Order execute: If you want execute order, enter into  Start Order and put order number to the link"""
    assert response_data['data'] == {'orders': [{
...
def test_check_product_on_order_positive(db_session, client, token):
    ...
    response = client.get(f"/api/v1/execute_order/check_order/{order_id}", headers={"Authorization": f"Bearer {token}"})
    response_data = response.json()
    assert response.status_code == 200
    assert response_data['data'] == {'Products': [{
...
def test_start_order_positive(db_session, client, token):
    ...
    create_service.create_random_order(3, shipping_date)
    order_id = db_session.execute(text('SELECT order_id FROM orders ORDER BY order_id DESC LIMIT 1')).scalar()
    response = client.get(f"/api/v1/execute_order/start_order/{order_id}", headers={"Authorization": f"Bearer {token}"})
    response_data = response.json()
...
def test_confirm_location(db_session, client, token):
    ...
    order = execute_service.Queue_To_Execute_Order(order_id)
    first = order[0]
    response = client.post(f"/api/v1/execute_order/confirm_location/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'location': first.location})
    response_data = response.json()
    assert response.status_code == 200
...
def test_confirm_location_wrong_location(db_session, client, token):
    ...
    order_id = db_session.execute(text('SELECT order_id FROM orders ORDER BY order_id DESC LIMIT 1')).scalar()
    location = 'RB-01-01'
    response = client.post(f"/api/v1/execute_order/confirm_location/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'location': location})
    response_data = response.json()
...
def test_confirm_ean(db_session, client, token):
    ...
    order = execute_service.Queue_To_Execute_Order(order_id)
    first = order[0]
    response = client.post(f"/api/v1/execute_order/confirm_ean/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'ean': first.ean})
    response_data = response.json()
    assert response.status_code == 200
...
def test_confirm_location_wrong_ean(db_session, client, token):
    ...
    first = order[0]
    ean = '1234567891234'
    response = client.post(f"/api/v1/execute_order/confirm_ean/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'ean': ean})
    response_data = response.json()
...
def test_confirm_amount_greater_than_location(db_session, client, token):
    ...
    response = client.post(f"/api/v1/execute_order/confirm_amount/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'amount': amount})
    response_data = response.json()
...
def test_get_next_product(db_session, client, token):
    ...
    response = client.get(f"/api/v1/execute_order/get_next_product/{order_id}", headers={"Authorization": f"Bearer {token}"})
    response_data = response.json()
    assert response.status_code == 200
    assert response_data['data']['product_name'] == first.product_name
    assert response_data['data']['amount to collect/amount available'] == f'{first.amount}/{amount_available}'
...
def test_show_done_products(db_session, client, token):
    ...
    collected = db_session.execute(text('SELECT collected_amount FROM orders_details WHERE order_id = :order_id AND ean = :ean'), {'order_id': order_id, 'ean': first.ean}).scalar()
    amount = first.amount - collected
    client.get(f"/api/v1/execute_order/start_order/{order_id}", headers={"Authorization": f"Bearer {token}"})
    client.post(f"/api/v1/execute_order/confirm_ean/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'ean': first.ean})
    client.post(f"/api/v1/execute_order/confirm_amount/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'amount': amount})
    response = client.get(f"/api/v1/execute_order/show_done_products/{order_id}", headers={"Authorization": f"Bearer {token}"})
    response_data = response.json()
...
def test_reverse_product_positive(db_session, client, token):
    ...
    client.post(f"/api/v1/execute_order/confirm_amount/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'amount': amount})
    product_id = 3
    response = client.post(f"/api/v1/execute_order/reverse_product/{order_id}", headers={"Authorization": f"Bearer {token}"}, json = {'product_id': product_id})
    response_data = response.json()
