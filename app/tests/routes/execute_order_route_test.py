...
def test_execute_order_choice_positive(db_session, client, token):
    ...
    orders = db_session.execute(
        text("""SELECT o.order_id, c.company_name, o.amount, o.total_weight FROM orders o
                        JOIN customers c ON o.customer_id = c.customer_id WHERE o.status = 'undone'""")
    ).fetchall()
    response = client.get(
        "/api/v1/execute_order/order_choice",
...
def test_check_product_on_order_positive(db_session, client, token):
    ...
    response = client.get(
        f"/api/v1/execute_order/check_order/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    response_data = response.json()
    assert response.status_code == 200
    assert response_data["data"] == {
        "Products": [{"Product name": row.product_name, "Amount": row.amount} for row in products]
    }
...
def test_start_order_positive(db_session, client, token):
    ...
    response = client.get(
        f"/api/v1/execute_order/start_order/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    response_data = response.json()
    ...
    amount_available = db_session.execute(
        amount_available_query,
        {"ean": first.ean, "location": first.location, "date": first.date},
    ).scalar()
    assert response.status_code == 200
    assert response_data["data"] == {
...
def test_confirm_location(db_session, client, token):
    ...
    response = client.post(
        f"/api/v1/execute_order/confirm_location/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
        json={"location": first.location},
    )
    response_data = response.json()
    assert response.status_code == 200
...
def test_confirm_location_no_order(db_session, client, token):
    ...
    response_data = response.json()
    assert response.status_code == 404
    assert response_data["detail"] == f"No products to pick up {order_id}"
...
def test_confirm_ean(db_session, client, token):
    ...
    first = order[0]
    response = client.post(
        f"/api/v1/execute_order/confirm_ean/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
        json={"ean": first.ean},
    )
    response_data = response.json()
...
def test_confirm_amount_greater_than_location(db_session, client, token):
    ...
    order = execute_service.Queue_To_Execute_Order(order_id)
    first = order[0]
    amount_on_location = db_session.execute(
        text("SELECT amount FROM products WHERE ean = :ean AND location = :location"),
        {"ean": first.ean, "location": first.location},
    ).scalar()
    amount = amount_on_location + 1
    response = client.post(
        f"/api/v1/execute_order/confirm_amount/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
        json={"amount": amount},
    )
    response_data = response.json()
    assert (
        response_data["message"] == f"It is only {amount_on_location} amount on location. Try again"
    )


def test_confirm_amount_greater_than_ordered(db_session, client, token):
    prepare_db(db_session)
    create_service = CreateOrder(db_session)
    execute_service = ExecuteOrder(db_session)
    create_service.create_random_order(3, shipping_date)
    ...
    collected = db_session.execute(
        text(
            "SELECT collected_amount FROM orders_details WHERE order_id = :order_id AND ean = :ean"
        ),
        {"order_id": order_id, "ean": first.ean},
    ).scalar()
...
def test_get_next_product(db_session, client, token):
    ...
    response = client.get(
        f"/api/v1/execute_order/get_next_product/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    response_data = response.json()
    assert response.status_code == 200
    assert response_data["data"]["product_name"] == first.product_name
    assert (
        response_data["data"]["amount to collect/amount available"]
        == f"{first.amount}/{amount_available}"
    )
    assert response_data["data"]["Location"] == first.location
    assert response_data["data"]["message"] == f"Confirm location {first.location}"
...
def test_show_done_products(db_session, client, token):
    ...
    response = client.get(
        f"/api/v1/execute_order/show_done_products/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    response_data = response.json()
    done = execute_service.get_done_products(order_id)
    assert response_data["data"]["Done products"] == [
        {
...
def test_reverse_product_positive(db_session, client, token):
    ...
    product_id = 3
    response = client.post(
        f"/api/v1/execute_order/reverse_product/{order_id}",
        headers={"Authorization": f"Bearer {token}"},
