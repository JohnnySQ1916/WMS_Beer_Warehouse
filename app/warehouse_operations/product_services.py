...
def get_current_amount(ean, location, db: Session, date=None):
    if date:
        query = text(
        ...
        query = text(
            "SELECT amount FROM products WHERE location = :location AND ean= :ean"
        )
        result = db.execute(query, {"location": location, "ean": ean}).scalar()
    return result


def update_location(old_location, ean, new_location, db: Session):
    query = text(
        "UPDATE products SET location = :new_location WHERE ean = :ean AND location = :old_location"
    )
    db.execute(
        query, {"new_location": new_location, "ean": ean, "old_location": old_location}
    )
    db.commit()
    print("Product location updated successfully.")


def update_amount(ean, location, added_amount, operation, db: Session):
    if operation == "sum":
        query = text(
        ...
        db.execute(
            query,
            {
                "new_amount": get_current_amount(ean, location) + added_amount,
                "ean": ean,
                "location": location,
            },
        )
...
def insert_new_product(amount, new_location, ean, location_choice, date, db: Session):
    query = text("""
...
def product_exist_on_location(new_location, ean, location_choice, date, db: Session):
...
def Execute_order(self, order_id, user_id, db: Session, min_date=None ):
    ...
    for product in order:
        ...
        self.db.execute(
            status_query, {"product_id": product.id, "order_id": order_id}
        ).scalar()
    ...
    order_query = text("UPDATE orders SET status = 'done' WHERE order_id = :order_id")
