...
class RelocationService:
    ...
    def new_record_relocation(self, ean: str) -> int:
        with transaction(self.db):
            ...
            new_record = self.db.execute(
                query, {'ean': ean, 'status': PickingStatus.EAN.value}).scalar()
            return new_record
    ...
    def confirm_location(self, relocation_id: int, location: str, date: date, user_id: str) -> None:
        with transaction(self.db):
            ...
            self.db.execute(query, {'location': location, 'date': date,
                            'status': PickingStatus.DATE.value, 'user_id': user_id, 'id': relocation_id})
    ...
    def update_date(self, product_id: int, relocation_id: int) -> None:
        with transaction(self.db):
            ...
            self.db.execute(query, {'product_id': product_id,
                    'status': PickingStatus.DATE.value, 'id': relocation_id})
    ...
    def confirm_amount(self, relocation_id: int, amount: int) -> None:
        with transaction(self.db):
            ...
            result = self.db.execute(
                amount_query, {'amount': amount, 'status': PickingStatus.AMOUNT.value, 'id': relocation_id})
    ...
    def confirm_target_location(self, relocation_id: str, target_location: str) -> None:
        with transaction(self.db):
            ...
            self.db.execute(query, {'target_location': target_location, 'time': datetime.now(timezone.utc).replace(microsecond=0), 'status': PickingStatus.DONE.value, 'id': relocation_id})

    def relocate_in_products(self, ean: str, location: str, date: date, amount: int, target_location: str) -> None:
        with transaction(self.db):
    ...
    def new_record_relocation_by_location(self, location: str, user_id: str) -> int:
        with transaction(self.db):
            ...
            new_id = self.db.execute(
                query, {'location': location, 'user_id': user_id,  'status': PickingStatus.LOCATION.value}).scalar()
            return new_id
    ...
    def confirm_ean(self, id: int, product_name: str, ean: str, date: date) -> None:
        with transaction(self.db):
            ...
            self.db.execute(query, {
                                        'product_name': product_name, 'ean': ean, 'date': date, 'status': PickingStatus.DATE.value, 'id': id})
