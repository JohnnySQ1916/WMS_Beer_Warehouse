...
class CreateOrder:
    ...
    def insert_into_orders(self, order_id: str) -> bool:
        with transaction(self.db):
            ...
            self.db.execute(insert_query, {"order_id": order_id, 'create_date': datetime.date.today(), 'status': OrderStatus.UNCONFIRMED.value})
            return True
    ...
    def update_status_orders(self, order_id: str) -> None:
        self.db.execute(text('UPDATE orders_details SET status = :status WHERE order_id = :order_id'), {'status': OrderStatus.UNCONFIRMED.value, 'order_id': order_id})
